...
SELECT 
aggregations.groupBy as orderId,
SUM(aggregations.totalAmountPaid) AS paid,
SUM(aggregations.shopify_discounts) AS discount,
SUM(aggregations.shopify_shippingcharges) AS shipping_charged,
SUM(aggregations.storehero_shippingcostwithouttax) AS shipping_cost,
SUM(aggregations.storehero_fulfillmentandpackagingcostcombined) AS fulfillment,
SUM(aggregations.shopify_successfulSaleTransactionFeesInShopCurrency) AS transactions,
SUM(aggregations.storehero_cogs) AS cogs,
SUM(aggregations.storehero_handlingfee_aka_packagingfee) AS packing_fees,
SUM(aggregations.storehero_grossprofit) AS gross_profit,
SUM(aggregations.storehero_grossprofitmargin) AS gpmargin,
SUM(aggregations.shopify_taxes) AS tax,
SUM(aggregations.storehero_productcost_factored_In_Toggle) AS productCost


FROM storeheroAggregations aggregations
GROUP BY orderId
