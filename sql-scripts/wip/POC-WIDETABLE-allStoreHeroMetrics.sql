...
SELECT
*,
null as productVariantId  --this is dummy for grouping
FROM
...
SELECT
null as productVariantId,  --this is dummy for grouping
null as orderId,  --this is dummy for grouping
...
null as productVariantId,  --this is dummy for grouping
null as orderId  --this is dummy for groupin
FROM
...
CAST(g.grouping as varchar) = CAST(t.grouping as varchar)
FULL Outer join metaAds_aggregation m on
CAST(g.grouping as varchar) = CAST(m.grouping as varchar)
FULL Outer join salesItemsAggregation si on
CAST(g.grouping as varchar) = CAST(si.grouping as varchar)
FULL Outer join aggregated_costScheduler cs on
CAST(g.grouping as varchar) = CAST(cs.grouping as varchar)
FULL Outer join transactionsAggregated trans on
CAST(g.grouping as varchar) = CAST(trans.grouping as varchar)

FULL Outer join fulfillmentsAggregated ful on
CAST(g.grouping as varchar) = CAST(ful.grouping as varchar)
