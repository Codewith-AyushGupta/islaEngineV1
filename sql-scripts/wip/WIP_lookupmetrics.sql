...
CREATE OR REPLACE TABLE api_fulfillments AS 
SELECT * FROM read_parquet('${s3BucketRoot}/tenants/tenantid=${tenantId}/transposed-output/tablename=model_fulfillments/*/*.parquet', hive_partitioning = true)
WHERE fulfillmentCreatedAtIsoYearOnly BETWEEN '2024' AND '2024';


--Partition needed?
-- they are fairly small
CREATE OR REPLACE TABLE api_transactions AS 
SELECT * FROM read_parquet('${s3BucketRoot}/tenants/tenantid=${tenantId}/transposed-output/tablename=model_transactions/*/*.parquet', hive_partitioning = true)
WHERE transactionCreatedAtIsoYearOnly BETWEEN '2024' AND '2024';
...
CASE 
    WHEN si.lineType = 'SHIPPING' AND si.actionType != 'RETURN' THEN 
   ...
   'countryCode_AllRegions', 
   	...
   	'shopify_shippingcharges', shopify_shippingcharges
 ...
 si.actionType,
    ...
    WHEN si.actionType = 'RETURN' THEN 0
 	...
 	ELSE json_extract_string(shippingCostMatrix, '$.shopify_shippingcharges') ::DOUBLE
...
f.legacyResourceId,
f.createdAt,
f.totalQuantity, --CAST TO DOUBLE
...
SELECT * FROM salesItemsDefinitions WHERE linetype = 'SHIPPING'
;
